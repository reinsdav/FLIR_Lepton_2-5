...
void ili9341_draw_image_RGB(ili9341_t *lcd, int16_t x, int16_t y, int16_t w, int16_t h, uint16_t* image)
{
  uint32_t num_pixels = w*h;
  uint32_t rect_wc    = num_pixels;
  uint32_t block_wc = rect_wc;
  ...
  for(int i = 0; i < 60; i++){
  	  for(int j = 0; j < 80; j++){
  		  line[j*4] = image[i*80 + j];
  		  line[j*4 + 1] = image[i*80 + j];
  		  line[j*4 + 2] = image[i*80 + j];
  		  line[j*4 + 3] = image[i*80 + j];
  	  }
  	  ili9341_transmit_color(lcd, 320 * 2, line, ibYes);
  	  ili9341_transmit_color(lcd, 320 * 2, line, ibYes);
  	  ili9341_transmit_color(lcd, 320 * 2, line, ibYes);
  	  ili9341_transmit_color(lcd, 320 * 2, line, ibYes);
  }
...
void ili9341_draw_image(ili9341_t *lcd, int16_t x, int16_t y, int16_t w, int16_t h, uint16_t* image)
{
  uint32_t num_pixels = w*h;
  uint32_t rect_wc    = num_pixels;
  uint32_t block_wc = rect_wc;
  ...
  //ili9341_transmit_color(lcd, block_wc * 2, image, ibYes);
  ...
  while (rect_wc > 0) {
    ...
    if (curr_wc > block_wc)
      { curr_wc = block_wc; }
    ili9341_transmit_color(lcd, curr_wc * 2, image, ibYes);
    rect_wc -= curr_wc;
  }
  */
